...
import { fetchContextsBilingual } from "../../scraper/client.js";
...
export class ContextSyncService {
  ...
  /**
   * Sync all contexts from INS API (bilingual)
   */
  async sync(): Promise<SyncResult> {
    const startTime = Date.now();
    logger.info("Starting context sync (bilingual)");

    // Fetch both languages in parallel
    const { ro: contextsRo, en: contextsEn } = await fetchContextsBilingual();
    logger.info(
      { roCount: contextsRo.length, enCount: contextsEn.length },
      "Fetched contexts from API (RO and EN)"
    );

    // Build EN name lookup map by code
    const enNameMap = new Map<string, string>();
    for (const ctx of contextsEn) {
      enNameMap.set(ctx.context.code, ctx.context.name);
    }

    // Sort by level (parents first)
    const sorted = this.topologicalSort(contextsRo);
    ...
    for (const ctx of sorted) {
      const nameEn = enNameMap.get(ctx.context.code) ?? null;
    ...
    logger.info(
      ...
      "Context sync completed (bilingual)"
    );
